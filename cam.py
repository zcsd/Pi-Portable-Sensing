...
sizeData = [ # Camera parameters for different size settings
 ...
 [(1920, 1080), (480, 320), (0.1296, 0.2222, 0.7408, 0.5556)], # Med
 [(1440, 1080), (480, 320), (0.2222, 0.2222, 0.5556, 0.5556)]] # Small
...
buttons = [
  ...
  [Button((  10,268,480, 52), bg='done' , cb=doneCallback),
  ...
  [Button((0,  0,480,320), cb=doneCallback), # Full screen = button
   Button((10,268,480, 52), bg='done'),       # Fake 'Done' button
   Button((0, 53,320, 80), bg='empty')],     # 'Empty' message

  # Screen mode 3 is viewfinder / snapshot
  [Button((  0,268,236, 52), bg='gear', cb=viewCallback, value=0),
   Button((244,268,236, 52), bg='play', cb=viewCallback, value=1),
   Button((  0,41,480,279)           , cb=viewCallback, value=2),
   Button(( 88, 51,267,142)),  # 'Working' label (when enabled)
...
def vidRange(path):
	...
	try:
	  for file in os.listdir(path):
	    if fnmatch.fnmatch(file, 'VID_[0-9][0-9][0-9][0-9].mp4'):
	      i = int(file[4:8])
	      if(i < min): min = i
	      if(i > max): max = i
	finally:
...
def takeVideo():
	global saveVIdx, busy
	
	r = vidRange("/home/pi/Videos")
	if r is None:
		saveVIdx = 1
	else:
		saveVIdx = r[1] + 1
		if saveVIdx > 9999:	saveVIdx = 0
	
	#scan for next available video slot	
	while True:
		mp4filename = '/home/pi/Videos' + '/VID_' + '%04d' % saveVIdx + '.mp4'
		if not os.path.isfile(mp4filename):
			h264filename =  '/home/pi/Videos' + '/VID_' + '%04d' % saveVIdx + '.h264'
			break
		saveVIdx += 1
		if saveVIdx > 9999: saveVIdx = 0
	...
	try:
		...
		call(["MP4Box", "-fps", "30", "-add", h264filename, mp4filename])
		call(["sudo", "rm", h264filename])
...
def drawBasic():
	...
	pygame.draw.rect(screen, GRAY, gasButton)
	switchText = pygame.font.Font("freesansbold.ttf",20)
	swtextSurf, swtextRect = text_objects("Gas", switchText, BLACK)
	swtextRect.center = (35, 20)
	screen.blit(swtextSurf, swtextRect)
	
	pygame.draw.rect(screen, GRAY, thermalButton)
	switch1Text = pygame.font.Font("freesansbold.ttf",15)
	sw1textSurf, sw1textRect = text_objects("Thermal", switch1Text, BLACK)
	sw1textRect.center = (451, 18)
	screen.blit(sw1textSurf, sw1textRect)
  ...
  while True:
    for event in pygame.event.get():
      if(event.type is MOUSEBUTTONDOWN):
        ...
        if selected(gasButton, pos):
			pygame.quit()
			camera.close()
			execfile("sensor.py")
        elif selected(thermalButton, pos):
            pygame.quit()
            camera.close()
            execfile("thermalCam.py")	
